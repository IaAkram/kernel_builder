...
suspatchesdir="$outside/ksu/hymo_patches/$(echo $kernel_ver | cut -d. -f1,2)"
...
sed -i "s/\(CONFIG_LOCALVERSION=\)\(.*\)/\1\"-${kernel_name}-ks${KSU_ver}hymo\"/" "${defconfig_file}"

echo "$(grep 'CONFIG_LOCALVERSION=' ${defconfig_file})"

echo -e " \nincludes backslashxx's KernelSU fork, ver ${KSU_ver}" >> banner_append
echo -e " \nincludes HymoFS" >> banner_append
